...
#   bash ghostype.sh publish [version]      编译 release + 打包 + 签名 + 公证 + 发布
#   bash ghostype.sh publish --no-notarize [version]  同上但跳过 Apple 公证
#   bash ghostype.sh dmg [version]          仅创建 DMG（需先 build release）
#
...
APPCAST_ROOT="${REPO_ROOT}/appcast.xml"
APPCAST_LOCAL="appcast.xml"
...
if [ -z "$COMMAND" ]; then
    ...
    echo "  publish [--no-notarize] [version]  编译 release + 打包 + 签名 + 公证 + 发布到 GitHub"
    echo "  dmg     [version]     仅创建 DMG（需先 build release）"
    exit 1
fi
...
bundle_app() {
    ...
    if [ "$CONFIG" = "debug" ]; then
        ...
        # Sparkle framework 内部组件需要单独签名（--timestamp 是公证必需的）
        ...
        # 主 app 签名（hardened runtime + timestamp + entitlements）
        codesign --force --options runtime --timestamp --entitlements "GHOSTYPE.entitlements" --sign "$SIGN_IDENTITY" \
...
#   2. bundle .app + Developer ID 签名
#   3. zip → Apple 公证 → staple
#   4. create-dmg 制作安装镜像
#   5. EdDSA 签名（Sparkle）
#   6. 生成 appcast.xml（repo root + 本地双写）
#   7. git commit + push appcast.xml
#   8. gh release create 上传 DMG
# ============================================================
...
cmd_publish() {
    ...
    # 解析参数：--no-notarize 和 version
    for arg in "$@"; do
        if [ "$arg" = "--no-notarize" ]; then
            NO_NOTARIZE=true
        elif [ -z "$VERSION" ]; then
            VERSION="$arg"
        fi
    ...
    # 读取当前 build number 并递增（优先从 repo root 的 appcast 读取）
    local CURRENT_BUILD=0
    if [ -f "${APPCAST_ROOT}" ]; then
        CURRENT_BUILD=$(sed -n 's/.*<sparkle:version>\(.*\)<\/sparkle:version>.*/\1/p' "${APPCAST_ROOT}" | head -1)
    elif [ -f "${APPCAST_LOCAL}" ]; then
        CURRENT_BUILD=$(sed -n 's/.*<sparkle:version>\(.*\)<\/sparkle:version>.*/\1/p' "${APPCAST_LOCAL}" | head -1)
    fi
    CURRENT_BUILD=${CURRENT_BUILD:-0}
    local NEW_BUILD=$((CURRENT_BUILD + 1))

    # 导出给 bundle_app / write_info_plist 使用
    export PUBLISH_VERSION="$VERSION"
    export PUBLISH_BUILD="$NEW_BUILD"

    if [ "$NO_NOTARIZE" = true ]; then
        echo "� GHOSTYPE Publish v${VERSION} (build ${NEW_BUILD}) [⚠️  跳过公证]"
    else
    ...
    # ── Step 2: 打包 + Developer ID 签名 ─────────────────────
    echo ""
    echo "📦 Step 2: Bundling + signing..."
    bundle_app release

    if [ "$NO_NOTARIZE" = true ]; then
        # 跳过公证，直接进入 DMG 制作
        echo ""
        echo "⏭️  Step 3-4: Skipped (--no-notarize)"
    else
        # ── Step 3: Apple 公证 ────────────────────────────────
        echo ""
        ...
        echo "   ZIP for notarization: $(du -h "${NOTARY_ZIP}" | cut -f1)"

        local NOTARY_OUTPUT
        local NOTARY_EXIT=0
        NOTARY_OUTPUT=$(xcrun notarytool submit "${NOTARY_ZIP}" \
            --keychain-profile "${NOTARY_PROFILE}" \
            --wait --timeout 30m 2>&1) || NOTARY_EXIT=$?
        echo "${NOTARY_OUTPUT}"
        ...
        if echo "${NOTARY_OUTPUT}" | grep -q "status: Accepted"; then
            ...
            echo ""
            echo "⚠️  Notarization failed or timed out (exit code: ${NOTARY_EXIT})"
            ...
            echo ""
            echo "   Once accepted, resume with:"
            echo "     xcrun stapler staple ${APP_BUNDLE}"
            echo "     bash ghostype.sh publish ${VERSION}"
            rm -f "${NOTARY_ZIP}"
            exit 1
        fi
    ...
    # ── Step 6: EdDSA 签名 (Sparkle) ─────────────────────────
    echo ""
    echo "🔐 Step 6: EdDSA signing..."
    ...
    if [ -z "$EDDSA_SIGNATURE" ]; then
        echo "❌ EdDSA signing failed"
        exit 1
    fi
    echo "   ✅ Signed"

    # ── Step 7: 生成 appcast.xml（双写：repo root + 本地）────
    echo ""
    echo "📄 Step 7: Generating appcast.xml..."
    local DOWNLOAD_URL="https://github.com/astronerd/ghostype/releases/download/v${VERSION}/${DMG_NAME}"
    local PUB_DATE
    PUB_DATE=$(date -R)

    local APPCAST_CONTENT
    read -r -d '' APPCAST_CONTENT << APPCAST_EOF || true
<?xml version="1.0" encoding="utf-8"?>
...
APPCAST_EOF
    ...
    # 写入 repo root（SUFeedURL 指向这里）
    echo "${APPCAST_CONTENT}" > "${APPCAST_ROOT}"
    echo "   ✅ ${APPCAST_ROOT}"

    # 写入本地备份
    echo "${APPCAST_CONTENT}" > "${APPCAST_LOCAL}"
    echo "   ✅ ${APPCAST_LOCAL}"

    # ── Step 8: Git commit + push ─────────────────────────────
    echo ""
    echo "📤 Step 8: Pushing appcast.xml..."
    cd "${REPO_ROOT}"
    git add appcast.xml AIInputMethod/appcast.xml
    git commit -m "release: v${VERSION} (build ${NEW_BUILD}) appcast"
    git push
    cd "${SCRIPT_DIR}"
    echo "   ✅ Pushed"
    ...
    echo ""
    echo "🏷️ Step 9: Creating GitHub Release..."
    cd "${REPO_ROOT}"
    gh release create "v${VERSION}" \
        "AIInputMethod/${DMG_NAME}" \
        --title "GHOSTYPE v${VERSION}" \
        --notes "GHOSTYPE v${VERSION} (build ${NEW_BUILD})" \
        --latest
    cd "${SCRIPT_DIR}"
    echo "   ✅ Released"
    ...
    echo "🧹 Cleaning up..."
    rm -f "${DMG_NAME}"
    echo "   ✅ Removed ${DMG_NAME}"

    echo ""
    echo "======================================================"
    echo "🎉 Published! GHOSTYPE v${VERSION} (build ${NEW_BUILD})"
