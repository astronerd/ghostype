...
enum MessageBuilder {
    ...
    static func buildProfilingUserMessage(
        ...
        // API 限制 10000 字符，预留 2000 给固定文案和 JSON 指令
        let maxCorpusChars = 5000
        let maxReportChars = 2000
        ...
        if let report = previousReport {
            if report.count > maxReportChars {
                parts.append(String(report.prefix(maxReportChars)) + "\n...(已截断)")
            } else {
                parts.append(report)
            }
        } else {
            parts.append("首次构筑，无历史报告")
        }
