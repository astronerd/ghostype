//
//  BearConfigView.swift
//  AIInputMethod
//
//  Bear 同步配置视图 - 默认标签、分组模式、标题模板、测试连接
//  Validates: Requirements 5.5, 6.1, 6.2, 8.3, 8.4, 8.5, 8.6
//
...
struct BearConfigView: View {
    ...
    init() {
        let existing = SyncConfigStore.shared.config(for: .bear)
            ?? SyncAdapterConfig(
                groupingMode: .perDay,
                titleTemplate: "GHOSTYPE Memo {date}",
                bearDefaultTag: "ghostype"
            )
    ...
    private var tagSection: some View {
        MinimalSettingsSection(title: L.MemoSync.defaultTag, icon: "tag") {
            HStack(spacing: DS.Spacing.md) {
                ...
                TextField("ghostype", text: Binding(
                    get: { config.bearDefaultTag ?? "ghostype" },
                    set: { newValue in
                        config.bearDefaultTag = newValue.isEmpty ? nil : newValue
                        saveConfig()
                    }
                ))
    ...
    private func testConnection() {
        ...
        Task {
            let result = await BearAdapter().validateConnection(config: config)
            await MainActor.run {
                switch result {
                case .success:
    ...
    private func saveConfig() {
        SyncConfigStore.shared.save(config: config, for: .bear)
    }
}
