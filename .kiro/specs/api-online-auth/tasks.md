...
- [x] 1. 完善 AuthManager 鉴权流程
  ...
  - [x] 1.2 在 AppDelegate 中注册 URL Scheme 回调
...
- [x] 2. 创建 GhostypeAPIClient 和数据模型
  ...
  - [x] 2.2 创建 GhostypeAPIClient 核心实现
  ...
  - [x] 2.3 编写 GhostypeAPIClient 属性测试
...
- [x] 3. Checkpoint - 鉴权和 API 客户端基础验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. 枚举迁移与数据兼容
  - [x] 4.1 迁移 PolishProfile 枚举 rawValue
  ...
  - [x] 4.3 创建 MigrationService 并在启动时执行
...
- [x] 5. Checkpoint - 枚举迁移验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. 重构 QuotaManager 使用服务端额度
  - [x] 6.1 重构 QuotaManager 为服务端字符额度
  ...
  - [x] 6.2 更新 QuotaManager 的 UI 调用方
  ...
  - [x] 6.3 编写 QuotaManager 属性测试
    - **Property 8: QuotaManager 状态与 ProfileResponse 一致**
    - **Validates: Requirements 7.2, 7.3, 7.4, 7.5**

- [x] 7. 接入调用链路：替换 GeminiService
  - [x] 7.1 修改 AppDelegate 润色流程
  ...
  - [x] 7.3 清理 GeminiService 中不再需要的代码
...
- [x] 8. Final checkpoint - 全流程验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. ASR 凭证云端迁移
  - [x] 9.1 修改 DoubaoSpeechService 使用缓存凭证 + 添加 fetchCredentials() 方法
  ...
  - [x] 9.2 在 AppDelegate 启动流程中集成 fetchCredentials() 调用
